...
.prob-bar-fill-covid    { height: 100%; background: linear-gradient(90deg,#e53935,#ef9a9a); border-radius: 50px; transition: width 0.6s ease; }
.prob-bar-fill-normal   { height: 100%; background: linear-gradient(90deg,#43a047,#a5d6a7); border-radius: 50px; transition: width 0.6s ease; }
.prob-bar-fill-pneumonia{ height: 100%; background: linear-gradient(90deg,#fb8c00,#ffcc80); border-radius: 50px; transition: width 0.6s ease; }
.prob-pct {
    width: 52px;
    text-align: right;
    font-size: 0.9rem;
...
4. Important note: this is AI assistance only, not a final diagnosis
...
<div class="hero">
    ...
    <p>Deep learning powered classification for COVID-19, Pneumonia & Normal cases</p>
</div>
""", unsafe_allow_html=True)
...
with c1:
    st.markdown('<div class="metric-box"><div class="metric-val">84%</div><div class="metric-lbl">Overall Accuracy</div></div>', unsafe_allow_html=True)
with c2:
    st.markdown('<div class="metric-box"><div class="metric-val">94%</div><div class="metric-lbl">COVID Accuracy</div></div>', unsafe_allow_html=True)
with c3:
    st.markdown('<div class="metric-box"><div class="metric-val">98%</div><div class="metric-lbl">Normal Accuracy</div></div>', unsafe_allow_html=True)
with c4:
    st.markdown('<div class="metric-box"><div class="metric-val">ResNet50</div><div class="metric-lbl">Best Model</div></div>', unsafe_allow_html=True)
...
with left:
    st.markdown('<div class="card"><h3>📤 Upload X-Ray</h3>', unsafe_allow_html=True)
...
with right:
    if uploaded and 'analyze' in dir() and analyze:
        ...
        with st.spinner("Analyzing X-ray..."):
            try:
                ...
                # Result badge
                st.markdown(f'<div class="result-{label}">{"🦠" if label=="covid" else "✅" if label=="normal" else "⚠️"} {label.upper()}</div>', unsafe_allow_html=True)
                st.markdown("<br>", unsafe_allow_html=True)
        ...
        <div class="card" style="text-align:center; padding: 60px 28px;">
            ...
            <div style="color:#546e7a; font-size:1rem; line-height:1.7;">
                Upload a chest X-ray image on the left<br>to begin AI-powered diagnosis
            </div>
        </div>
        """, unsafe_allow_html=True)
